...
const OrderForm = ({ orderItems, setOrderItems }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      alert(`Your order has been placed successfully. Order ID: ${result.data.placeOrder.id}`);
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <ul>
        {orderItems.map((item) => (
          <li key={item.id}>
            ...
            <input type="number" id={item.name} value={item.quantity} min="0" max="10" step="1" onChange={(event) => setOrderItems([...orderItems, { ...item, quantity: parseInt(event.target.value, 10) }])} />
          </li>
        ))}
      </ul>
